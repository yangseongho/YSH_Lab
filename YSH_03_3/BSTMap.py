...
if __name__ == "__main__":
    ...
    map = BSTMap()
    map.display("[삽입 전] : ", order=1)  # 초기 상태 출력 (inorder)

    for i in range(len(data)):
        map.insert(data[i], value[i])
        map.display(f"[삽입 {data[i]}] : ", order=1)  # 삽입 후 중위순회 (inorder)

    # 순회 방식에 따른 출력
    print("======== 순회 방식 테스트 ========")
    print("In-Order 순회:")
    map.display("[중위순회] : ", order=1)  # 중위순회
    print("Pre-Order 순회:")
    map.display("[전위순회] : ", order=2)  # 전위순회
    print("Post-Order 순회:")
    map.display("[후위순회] : ", order=3)  # 후위순회
    ...
    # 삭제 테스트
    map.delete(3)
    map.display("[삭제  3] : ", order=1)
    map.delete(68)
    map.display("[삭제 68] : ", order=1)
    map.delete(18)
    map.display("[삭제 18] : ", order=1)
    map.delete(35)
    map.display("[삭제 35] : ", order=1)
