...
def delete_node(root, key):
    ...
    if key < root.data:
        ...
        # 3. key가 root의 data와 같으면 삭제할 노드를 찾음
        # (1) 자식이 없는 경우
        ...
        # (3) 자식이 두 개 있는 경우
        # 오른쪽 서브트리에서 가장 작은 값을 찾음 (혹은 왼쪽 서브트리에서 가장 큰 값을 찾음)
        min_node = find_min(root.right)
        root.data = min_node.data  # 가장 작은 값을 현재 노드에 복사
        root.right = delete_node(root.right, min_node.data)  # 오른쪽 서브트리에서 그 값을 삭제
...
def testBinaryTree():
    ...
    print('\n초기 트리 상태 (Pre-Order): ', end='')
    preorder(root)
    print("\n")

    # 노드 'D' 삭제 테스트
    print("노드 'D' 삭제 후 트리 상태 (Pre-Order): ", end='')
    delete_node(root, 'D')
    preorder(root)
    print("\n")

    # 노드 'B' 삭제 테스트
    print("노드 'B' 삭제 후 트리 상태 (Pre-Order): ", end='')
    delete_node(root, 'B')
    preorder(root)
    print("\n")

    # 노드 'A' 삭제 테스트 (루트 노드 삭제)
    print("노드 'A' 삭제 후 트리 상태 (Pre-Order): ", end='')
    delete_node(root, 'A')
    preorder(root)
    print("\n")
